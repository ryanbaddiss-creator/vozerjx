...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang4/lacochienthang.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(LACOCHIENTHANG) < nGioiHanEventDacBiet then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbLaCoChienThang,1)
					SetTask(LACOCHIENTHANG,GetTask(LACOCHIENTHANG)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventDacBiet.." L¸ cê chiÕn th¾ng Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
