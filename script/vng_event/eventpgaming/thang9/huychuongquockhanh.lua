...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang9/huychuongquockhanh.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(HUYCHUONGQUOCKHANH) < nGioiHanEventDacBiet then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbHuyChuongQuocKhanh,1)
					SetTask(HUYCHUONGQUOCKHANH,GetTask(HUYCHUONGQUOCKHANH)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventDacBiet.." Huy ch­¬ng quèc kh¸nh Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
