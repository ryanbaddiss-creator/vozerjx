...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang1/bcthuonghang.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHCHUNGTHUONGHAN) < nGioiHanEventDacBiet then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhChungThuongHan,1)
					SetTask(BANHCHUNGTHUONGHAN,GetTask(BANHCHUNGTHUONGHAN)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventDacBiet.." B¸nh ch­ng th­îng h¹n Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
