...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang1/bchaohang.lua")
-------------------------------------
...
SetTask(BANHCHUNGHAOHAN, 0)   ;SetTask(PHONGPHAOTRUNGDB, 0)    ;																																																	   SetTask(BANHTRUNGTHUDB, 0)       ;							 SetTask(NGUOITUYETDB, 0)              ;
							   SetTask(PHONGPHAOTRUNGTHUONG, 0);																																																	   SetTask(BANHTRUNGTHUNHANTRUNG, 0);
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(BANHCHUNGHAOHAN) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbBanhChungHaoHan,1)
					SetTask(BANHCHUNGHAOHAN,GetTask(BANHCHUNGHAOHAN)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." B¸nh Ch­ng H¶o H¹n Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
		else
