...
----------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang2/phongphaotrungthuong.lua")
-------------------------------------
...
return 1 end
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
			if GetTask(PHONGPHAOTRUNGTHUONG) < nGioiHanEventThuong then
				if CalcFreeItemCellCount() >= 10 then
					tbAwardTemplet:GiveAwardByList(tbPhongPhaoTrungThuong,1)
					SetTask(PHONGPHAOTRUNGTHUONG,GetTask(PHONGPHAOTRUNGTHUONG)+1)
				else
					Say("H·y cÊt bít vËt phÈm ®Ó ®¶m b¶o cã 10 « trèng råi h·y sö dông",0) return 1
				end
			else
				Say("Mçi Nh©n VËt ChØ Sö Dông Tèi §a "..nGioiHanEventThuong.." Phong Ph¸o Trung Th­êng Trong Suèt Thêi Gian Ho¹t §éng") return 1
			end
	else
