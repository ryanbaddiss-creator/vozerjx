...
--------------------------------------------------------------
function main()
dofile("script/vng_event/eventpgaming/thang5/npc_sukien.lua")
...
local nTime1 = "20"..nYear.."05010000"
local nTime2 = "20"..nYear.."06010000"
...
local nDayNow = "20"..nYMD..""
	if (nDayNow >= nTime1) and (nDayNow <= nTime2) then
		...
		Talk(1,"","<color=green>Hoπt ßÈng Di‘n Ra Tı:\n\n <color=red>0h Ngµy 01 - 05 - 20"..nYear.." ß’n 0h Ngµy 01 - 06 - 20"..nYear.."<color><color>")
	end
end				
------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
function NhanMoc()
	local tbOpt = {
		{"NhÀn Th≠Îng MËc "..nGioiHanMoc1.." B∏nh chay Æ∆c bi÷t",PhanThuong1},
		{"NhÀn Th≠Îng MËc "..nGioiHanMoc2.." B∏nh chay Æ∆c bi÷t",PhanThuong2},
		{"NhÀn Th≠Îng MËc "..nGioiHanMoc3.." B∏nh chay Æ∆c bi÷t",PhanThuong3},
		{"HÒy B·",},
	}
	CreateNewSayEx("<color=green>"..myplayersex().." MuËn NhÀn MËc Nµo<color>\n<color=red>Hi÷n Tπi Ng≠¨i ß∑ Sˆ DÙng <color=yellow>"..GetTask(BANHCHAYDB).."<color> B∏nh chay Æ∆c bi÷t<color>", tbOpt)
end	
-----------------------------------------------------Moc 500-------------------------------------------------------------------------------------------------------------------------------------------
function PhanThuong1()
	if (GetTask(NHANTHUONGMOC1) == 0) then
		if (GetTask(BANHCHAYDB) < nGioiHanMoc1) then
...
function PhanThuong2()
	if (GetTask(NHANTHUONGMOC2) == 0) then
		if (GetTask(BANHCHAYDB) < nGioiHanMoc2) then
...
function PhanThuong3()
	if (GetTask(NHANTHUONGMOC3) == 0) then
		if (GetTask(BANHCHAYDB) < nGioiHanMoc3) then
