---------------Youtube PGaming---------------
Include("\\script\\vng_event\\eventpgaming\\thang5\\head.lua");
...
--			if (GetSkillState(765) == 1) then		--OuÔÚÉO±u×´`¬
--				Msg2Player("OuÔÚÄA³öÉO±uÖÐ£¬ÇëÉÔµÈ£¡");
--			else																--²»ÊÇÉO±u×´`¬
				...
				if (nparam4 > 0 and nmyTime == 0) then
					...
					if (bud > 0) then
						szmsg = format("%s <color=yellow>%d c¸i<color> B¸nh chay ch­a chUn, ",szmsg, bud);
					end;
...
--			end;
			...
			if (task >= 1 and task <= 4) then		--ÓÐ?»¸öÈÎÎñ
				if (nstate == 0) then	--»¹A»½Ó
					Say(DEC_PAN_EVENT[task], 1, "Ta biÕt råi/#sure2pantaketask("..nNpcIdx..")");
				elseif (nstate == 1) then	--½ÓÁË£¬A»×ö
...
function sure2pickpan(nNpcIdx)
	...
	if (nparam4 > 0) then
		...
		Msg2Player("§îi 3 gi©y sau míi ca thÓ vít b¸nh ra.");
	else
		Talk(1, "", DEC_PAN_OTHER[random(getn(DEC_PAN_OTHER))]);
		Msg2Player("B¸nh ®· vít ra hOt råi!");
	end;
end;
...
function sure2pantaketask(nNpcIdx)
	...
	if (task >= 1 and task <= 4 and nstate == 0) then
		...
		AddNpcSkillState(nNpcIdx, 662, 1,1, 0);	--Çå `¾ºÅ
...
function sure2pandotask(nNpcIdx)
	...
	if (task >= 1 and task <= 4 and nstate == 1) then
		...
		elseif (GetSex() ~= nsex) then
			local szmsg = format("ChØ ca %s míi ca thÓ %s ®­îc.", DEC_PAN_STASK[task], DEC_PAN_SZSEX[nsex]);
			Say(szmsg, 0);
		end;
	end;
end;
...
function OnTimer(nNpcIdx, nTimeOut)
	...
	if (nphase == 4) then
		...
		if (nevent == 10) then	--±äÉíÊ±¼ä
			...
			nphase = nphase + 1;
			nstate = 0;
			local nx, ny = GetNpcPos(nNpcIdx);	--ÉO±u¹øÀï×ø±ê
			local szname = GetNpcName(nNpcIdx);			--ÉO±u¹øÀïµÄAû×Ö
			DelNpc(nNpcIdx);										--É¾µô¸AÉO±u¹øÀï
																					--´´½¨?»¸ö³É³¤ºóµÄÉO±u¹øÀï
			local nIdx = AddNpc(TB_PAN_NPCID[nphase], 1, SubWorld, nx, ny, 1, szname);
			
				--¸´ÖÆÔ­À´¹øµÄÐÅÏ¢
			SetNpcScript(nIdx, "\\script\\vng_event\\eventpgaming\\thang5\\pan.lua");
			...
			if (nphase ~= 4) then
				SetNpcTimer(nIdx, 30*18);	--¿ªÆôÏÂ?»¸ö¶¨Ê±Æ÷
			else
				initPANCount(nIdx, npoint);
				SetNpcTimer(nIdx, 30*60*18);	--¿ªÆôÏÂ?»¸ö¶¨Ê±Æ÷
			end;
		else									--³É³¤Ê±¼ä
			...
			if (process == 1) then		--¿ªÆô?»¸öÈÎÎñ
				startPANTask(nNpcIdx, nphase);
			elseif (process == 2) then		--¿ªÆô?»¸öÈÎÎñ
				if (nstate == 0) then
					endPANTask(nNpcIdx);
				end;
			elseif (process == 0) then	--½áÊø?»¸öÈÎÎñ
				endPANTask(nNpcIdx);
			end;
			SetNpcTimer(nNpcIdx, 30*18);	--¿ªÆôÏÂ?»¸ö¶¨Ê±Æ÷
		end;
	end;
end;
...
function startPANTask(nNpcIdx, nphase)
	local task = random(1, TB_PAN_TASK[nphase]);	--Ëæ»ú?»¸öÈÎÎñ
	SetNpcParam(nNpcIdx, PRM_PAN_EVENT, SetByte(GetNpcParam(nNpcIdx, PRM_PAN_EVENT), 4, task));	--´¥·¢ÈÎÎñ
	AddNpcSkillState(nNpcIdx, 662,1, 1, 30*18);	--ÏÔÊ¾ÈÎÎñ×´`¬
	
end;

function endPANTask(nNpcIdx)
	AddNpcSkillState(nNpcIdx, 662,1, 1, 0);	--ÇåµôÏÔÊ¾×´`¬
	AddNpcSkillState(nNpcIdx, 765,1, 1, 0);	--ÇåµôÏÔÊ¾×´`¬
	SetNpcParam(nNpcIdx, PRM_PAN_EVENT, SetByte(GetNpcParam(nNpcIdx, PRM_PAN_EVENT), 2, 0));
	SetNpcParam(nNpcIdx, PRM_PAN_EVENT, SetByte(GetNpcParam(nNpcIdx, PRM_PAN_EVENT), 4, 0));
	SetNpcParam(nNpcIdx, PRM_PAN_TIME, 0);
