...
--Ö÷¶Ô»°¿ò
function main()
	if NPCVoLamLienDau ~= 1 then
		return Talk(1, "", "<color=Orange>Sø Gi¶ Liªn §Êu: <color>HiÖn t¹i tİnh n¨ng nµy vÉn ch­a më!")	
	end
	local n_next = GetGlbValue(GLB_WLLS_NEXT)
	if n_next ==0 then
		Say("Tİnh n¨ng ch­a ®­îc më ",0)
		return
	end
	...
	local n_next = GetGlbValue(GLB_WLLS_NEXT)
	local str = "<color=Orange>Sø Gi¶ Liªn §Êu:<color> "..WLLS_TAB[n_next].text_main[n_level]
	local str_des = WLLS_LEVEL_DESC[n_level]
	local tb_option = {}
	
	if (GetGlbValue(GLB_WLLS_PHASE) == 1) then
		tb_option[getn(tb_option) + 1] = "-> Ta muèn nhËn th­ëng kÕt thóc mïa gi¶i!/wlls_wantaward"
	end
	...
	tb_option[getn(tb_option) + 1] = "Ta muèn l·nh phÇn th­ëng liªn ®Êu/wlls_buy_honour" ----
	-- tb_option[getn(tb_option) + 1] = "Ta muèn kiÓm tra ®iÓm Uy Danh/wlls_ShowRespect"
	-- tb_option[getn(tb_option) + 1] = "§ãng/ m?nhËn kinh nghiÖm/wlls_show_expswitch"
...
function wlls_wantaward()
	...
	local str = "<color=Orange>Sø Gi¶ Liªn §Êu:<color> BÊt kÓ tham gia Vâ l©m liªn ®Êu   hay vâ l©m kiÖt xuÊt ®Òu ®­îc nhËn phÇn th­ëng. PhÇn  th­ëng bao gåm <color=red>PhÇn th­ëng xÕp h¹ng<color> vµ <color=red>®iÓm Vinh dù<color>.  Sau mçi giai ®äan thi ®Êu kÕt thóc, sÏ c¨n cø vµo <color=yellow>®iÓmtİch lòy, tû lÖ th¾ng, tæng thêi gian thi ®Êu<color> ®Ó  xÕp h¹ng. <color=green>"..n_awardmin.." ®éi xÕp cao nhÊt<color> sÏ nhËn ®­îc <color=red>phÇn th­ëng   ®Æc biÖt<color>. Ng­¬i muèn nhËn phÇn th­ëng g×?"
	local tb_option = {}
...
function wlls_wantaward_rank()
	...
	if nAward then
		...
		local str = "<enter> ChiÕn ®éi cña b¹n:<color=yellow> "..szLGName.."<color> Trong lÇn thi ®Êu "..str_des.." nµy ®­îc xÕp h¹ng <color=yellow>"..nRank.."<color>. Ng­¬i cã thÓ nhËn ®­îc ®iÓm vinh dù <color=green>"..tb_award[2].."<color> ®iÓm."
...
function wlls_wantaward_title()
	...
	local tb = {"Qu¸n qu©n", "¸ qu©n", "H¹ng 3", "H¹ng 4"}
...
function wlls_want2signmap()
	...
	if (n_matchphase < 2) then
		...
		Say("<color=Orange>Sø Gi¶ Liªn §Êu: <color>Hiªn t¹i lµ thêi gian liªn ®Êu t¹m nghØ, héi tr­êng sÏ t¹m ®ãng cöa, b¹n h·y chê ®Õn kú thi ®Êu h·y quay l¹i!", 0)
		return 
	end
	
	if GetLevel() < WLLS_LEVEL_JUNIOR then
		return Say("<color=Orange>Sø Gi¶ Liªn §Êu: <color><color=red>Vâ l©m liªn ®Êu<color> vÉn lµ n¬i Anh   hïng thiªn h¹ luËn kiÕm, ®¼ng cÊp cña ng­¬i vÉn ch­a ®ñ <color=yellow>"..WLLS_LEVEL_JUNIOR.." cÊp<color>, anh hïng kh¾p n¬i tÒ tô, ngäa hæ tµng longe r»ng ng­¬i chØ lµ kÎ hËu bèi! Tèt h¬n nªn luyÖn tËpthªm!")
	end
		
	-- local n_level, n_group = wlls_getcityinfo()
	-- local n_level, _ = wlls_getcityinfo()
	if GetLevel() < 120 then
		-- n_level = 1
		n_level = 1
	else
	...
	local n_group = 1 --Set tÊt c¶ c¸c nhãm tËp trung l¹i 1 chç-> §¹i l? cao cÊp - BiÖn kinh s¬ cÊp
	...
	--´«ËÍµ½±¨ÃûµãµØÍ¼
	local MapType = "<color=green>B¸o danh<color>";
	if n_level == 1 then
		-- MapType = "<color=green>80 -> 119<color>";
	elseif n_level == 2 then
		-- MapType = "<color=pink>Cao CÊp<color>";
	end
	local n_signmap = wlls_get_mapid(1, n_mtype, n_group)
	NewWorld(n_signmap, WLLS_MAPPOS_SIGN[1], WLLS_MAPPOS_SIGN[2])
	Msg2Player("B¹n ®· ®Õn khu vùc thi ®Êu <enter>           "..MapType.." - <color=yellow>"..wlls_get_desc(3, n_mtype, n_group).."<color>")
...
function wlls_buy_honour()
	...
	Talk(1, "BuyiTemWithHonour", "<enter><color=Orange>"..GetName().."<color> "..SexStr.." hiÖn ®ang cã<color=yellow> "..GetTask(WLLS_TASKID_HONOUR).."<color> ®iÓm vinh dù");
	function BuyiTemWithHonour()
		if (GetBoxLockState() == 0) then
			-- Sale(146,11) -- ÉÌµêµÄÈÙÓş»ı·Ö±àºÅÎª11
	
			if ShopLienDau == 0 and ScriptShopLienDau == 1 then
				shopliendau22()
			elseif ShopLienDau == 0 then
			else
			-- return Talk(1, "", "HiÖn t¹i cöa hµng Liªn ®Êu ®ang trong qu¸ tr×nh thu thËp th«ng tin, §éc c« cÇu b¹i sÏ më l¹i cöa hµng nµy trong thêi gian sím nhÊt cã thÓ!")
			Sale(146,11)
			end
		else
